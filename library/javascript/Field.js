...
class Field {                    // class for DataForm2.0
    ...
    getField = function() {
        let fieldHTML = "", tmpValueArry = [], tmpType, i, l;
        if( this.opt.type.substring( 0, 6 ) === "input_" ) {
            this.opt.type = this.opt.type.split( "_" )[0]
        }
        ...
        switch ( this.opt.type) {
            case "select":
                ...
                break;
            case "input":
                tmpType = this.opt.type.split( "_" )[1]
                if( typeof this.opt.index !== "undefined" ) {
                    fieldHTML += '<input type="' + tmpType + '" id="' + this.opt.id + '_' + this.opt.index + '" ';
                } else {
                    fieldHTML += '<input type="' + tmpType + '" id="' + this.opt.id + '" ';
                }
                fieldHTML += '>';
                this.tmpEl = htmlToElement( fieldHTML );
                break;
