...
//
            nj(e).sty({ left: parseInt( parent.width )  - 20 + "px", top: parseInt( parent.height ) + "px"} );  // + footer height
        break
    }
...
function re_dragAction(e, t) {
        ...
        //let el_dialog_height = parseInt( re_getElementSizeAndPosition( nj().els( "#" + e.param[0].id ).previousSibling.previousSibling ).height );
        let res_top = parseInt( resizer_pos.top );
        const dia_hl_height = 35 + 3 // hl + menu
        const dia_footer_height = 40;
        let testvalue = res_top -  parseInt( c.top ) - dia_hl_height - dia_footer_height;
        console.log( res_top, c.height, dia_hl_height, testvalue );
        nj( nj().els( "#" + e.param[0].id ).previousSibling.previousSibling ).sty( "height", testvalue + "px" );
}
